...
def registrar_execucao(aeroportos: list, voos_processados: int, lotes_enviados: int, erros: int, status: str, obs: str = "") -> None:
    try:
...
if not todos_voos:
    ...
    registrar_execucao(AIRPORTS, 0, 0, 0, "sem_dados", "API SIROS não retornou voos para a data.")
    sys.exit(0)
...
unicos = {}
for r in registros:
    chave = (r["data_referencia"], r["icao_empresa"], r["numero_voo"], r["icao_origem"], r["icao_destino"], r["etapa"])
    unicos[chave] = r
...
obs = f"Data: {data_iso} | Aeroportos: {', '.join(AIRPORTS)} | Processados: {total_processados} | Lotes: {total_lotes} | Erros: {total_erros}"

registrar_execucao(AIRPORTS, total_processados, total_lotes, total_erros, status_final, obs)
